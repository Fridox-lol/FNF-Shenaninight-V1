...
class AchievementsMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		if(FlxG.save.data.achievementsUnlocked != null)
			percentage = Math.round((FlxG.save.data.achievementsUnlocked.length / 30) * 100);
		trace(percentage);
		...
		nameText.setFormat(Paths.font("phantom.ttf"), 32, FlxColor.WHITE, CENTER);
		nameText.scrollFactor.set();

		descText = new FlxText(50, nameText.y + 38, FlxG.width - 100, "", 24);
		descText.setFormat(Paths.font("phantom.ttf"), 24, FlxColor.WHITE, CENTER);
		...
		progressTxt.setFormat(Paths.font("phantom.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		progressTxt.scrollFactor.set();
		progressTxt.borderSize = 2;

		var percentext:FlxText = new FlxText(10, 60, 0, "Progress: " + percentage + "%", 32);
		percentext.setFormat(Paths.font("phantom.ttf"), 32, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		percentext.scrollFactor.set();
...
class ResetAchievementSubstate extends MusicBeatSubstate
{
	...
	public function new()
	{
		...
		text.setFormat(Paths.font("phantom.ttf"), 40, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		text.scrollFactor.set();
		text.borderSize = 2;
		add(text);
