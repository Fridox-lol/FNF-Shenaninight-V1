...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		["Absolutely fucking horrible", 0.2], //From 0% to 19%
		["...seriously?", 0.4], //From 20% to 39%
		["Horrible", 0.5], //From 40% to 49%
		["Not so great", 0.6], //From 50% to 59%
		["Mmmeh", 0.69], //From 60% to 68%
		["Noice", 0.7], //69%
		["Cool!", 0.8], //From 70% to 79%
		["Lit!", 0.9], //From 80% to 89%
		["Amazing!!", 1], //From 90% to 99%
		["ABSOLUTELY PERFECT!", 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	public static var isFNFVerse:Bool = false;
	...
	override public function create()
	{
		...
		botplayTxt.setFormat(Paths.font("phantom.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	public function endSong()
	{
		...
		if (controls.controllerMode)
			{ 
				trace(controls.controllerMode);
				Achievements.unlock("controllerPlay");
			}

		if (songName == "HYPERNOVA")
			{ //toby fox ahh coding 2: electric boogaloo
				if (FlxG.save.data.snName == "LORI")
					Achievements.unlock("namebeat0001");
				
				else if (FlxG.save.data.snName == "FRIDOX")
					Achievements.unlock("namebeat0001");

				else if (FlxG.save.data.snName == "DONKEY")
					Achievements.unlock("namebeat0001");

				else if (FlxG.save.data.snName == "SHELBY")
					Achievements.unlock("namebeat0001");

				else if (FlxG.save.data.snName == "SIMONTINCS")
					Achievements.unlock("namebeat0002");

				else if (FlxG.save.data.snName == "DYLAN")
					Achievements.unlock("namebeat0003");

				else if (FlxG.save.data.snName == "GALAXTOR")
					Achievements.unlock("namebeat0004");

				else if (FlxG.save.data.snName == "LAPTOX")
					Achievements.unlock("namebeat0005");

				else if (FlxG.save.data.snName == "STARI")
					Achievements.unlock("namebeat0005");

			}
			else
			{
				trace((songName == "HYPERNOVA") + ", song name is " + songName);
				// trace(songName == "HYPERNOVA");
			}

		if (FlxG.save.data.achievementsUnlocked.length == 29)
			if (FlxG.save.data.achievementsUnlocked != null)
				Achievements.unlock('all_of_em');
		...
		checkForAchievement([weekNoMiss, 'ur_bad', 'ur_good', 'failure', 'noice']);
		#end
		...
		if(ret != FunkinLua.Function_Stop && !transitioning)
		{
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if (FlxG.save.data.achievementsUnlocked.length == 30)
						FlxG.sound.playMusic(Paths.music('freakyMenuPost'));
					else
			...
			else
			{
				...
				if(isFNFVerse)
				{
					...
					FlxG.sound.playMusic(Paths.music('saateotu'));
				}
				else
				{
	...
	private function checkForAchievement(achievesToCheck:Array<String> = null)
	{
		...
		for (name in achievesToCheck) {
			...
			if (name != WeekData.getWeekFileName() + '_nomiss') // common achievements
			{
				...
				{
					case 'failure':
						unlock = (ratingPercent <= 0.1 && !practiceMode);

					case 'ur_bad':
						unlock = (ratingPercent < 0.2 && !practiceMode);

					case 'noice':
						unlock = ((ratingPercent >= 0.69 && ratingPercent <= 0.7) && !practiceMode);
			...
			else // any FC achievements, name should be "weekFileName_nomiss", e.g: "week3_nomiss";
			{
				...
				if (isStoryMode && campaignMisses + songMisses < 1 && Difficulty.getString().toUpperCase() == 'HARD' && storyPlaylist.length <= 1 && !changedDifficulty && !usedPractice && WeekData.getWeekFileName() == "week1")
					Achievements.unlock('week1_nomiss_throw');
			}
