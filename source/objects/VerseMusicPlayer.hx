 ...
 */
@:access(states.FNFVerseState)
class VerseMusicPlayer extends FlxGroup 
{
	public var instance:FNFVerseState;
	...
	public function new(instance:FNFVerseState)
	{
		super();
	...
	override function update(elapsed:Float)
	{
		...
		if (paused && !wasPlaying)
			songTxt.text = 'PLAYING: ' + instance.songs[FNFVerseState.curSelected].songName + ' (PAUSED)';
		else
			songTxt.text = 'PLAYING: ' + instance.songs[FNFVerseState.curSelected].songName;
		...
		if (instance.controls.UI_LEFT_P)
		{
			...
			if (FNFVerseState.vocals != null)
				FNFVerseState.vocals.time = curTime;
		}
		...
		if (FNFVerseState.vocals != null && FlxG.sound.music.time > 5)
		{
			var difference:Float = Math.abs(FlxG.sound.music.time - FNFVerseState.vocals.time);
			if (difference >= 5 && !paused)
			{
				pauseOrResume();
				FNFVerseState.vocals.time = FlxG.sound.music.time;
				pauseOrResume(true);
			}
		}
		updatePlaybackTxt();
	...
	{
		var length:Int = instance.songs[FNFVerseState.curSelected].songName.length;
	...
	function setPlaybackRate() 
	{
		...
		if (FNFVerseState.vocals != null)
			FNFVerseState.vocals.pitch = playbackRate;
	}
